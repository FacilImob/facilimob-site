...
export async function sendLoginCodeEmail({ to, code }) {
  ...
  await transporter.sendMail({
    ...
    to,
    subject: 'Seu codigo de acesso ao simulador FacilImob',
    html: loginCodeHtml(code),
    text: `Seu codigo de acesso ao simulador FacilImob e ${code}. Ele expira em 10 minutos.`
  });
}

function loginCodeHtml(code) {
  const appUrl = process.env.PUBLIC_APP_URL || 'https://garantias.imobiliariainglaterra.com.br';
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f7fb;padding:28px 14px;">
      <tr><td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border:1px solid #d7e1ec;border-radius:16px;overflow:hidden;">
          ...
          <tr><td style="padding:30px 30px 8px;text-align:center;"><img src="${escapeHtml(appUrl)}/assets/logo-facilimob-horizontal-cropped.png" width="220" alt="FacilImob" style="max-width:220px;width:100%;height:auto;border:0;"></td></tr>
          <tr><td style="padding:18px 30px 30px;">
            <h1 style="margin:0 0 12px;font-size:24px;line-height:1.25;color:#004477;">Codigo de acesso ao simulador</h1>
            <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#44546a;">Use o codigo abaixo para entrar no simulador FacilImob.</p>
            ...
            <p style="margin:0 0 18px;text-align:center;"><a href="${escapeHtml(appUrl)}/login.html" style="display:inline-block;background:#ff7900;color:#ffffff;text-decoration:none;font-weight:700;font-size:15px;padding:12px 20px;border-radius:10px;">Abrir simulador</a></p>
